...
const erlang_types_1 = __importDefault(require("erlang-types"));
class ProcessSystem {
    constructor() {
        this.pids = new Map();
        ...
        this.scheduler = new scheduler_1.default(throttle);
    ...
    monitor(pid) {
        ...
        if (real_pid) {
            ...
            this.send(this.current_process.pid, new erlang_types_1.default.Tuple('DOWN', ref, pid, real_pid, Symbol.for('noproc')));
            return ref;
        }
    }
    demonitor(ref) {
    ...
    set_current(id) {
        ...
        if (pid !== null) {
            ...
            this.current_process.status = states_1.default.RUNNING;
        }
    }
    add_proc(fun, args, linked, monitored) {
        let newpid = new erlang_types_1.default.PID();
        let mailbox = new mailbox_1.default();
        let newproc = new process_1.default(newpid, fun, args, mailbox, this);
    ...
    pidof(id) {
        if (id instanceof erlang_types_1.default.PID) {
    ...
    suspend(fun) {
        this.current_process.status = states_1.default.SUSPENDED;
        this.suspended.set(this.current_process.pid, fun);
    }
    delay(fun, time) {
        this.current_process.status = states_1.default.SLEEPING;
    ...
    exit(one, two) {
        ...
        if (two) {
            ...
            if ((process && process.is_trapping_exits()) || reason === states_1.default.KILL || reason === states_1.default.NORMAL) {
                this.mailboxes.get(process.pid).deliver(new erlang_types_1.default.Tuple(states_1.default.EXIT, this.pid(), reason));
            }
            else {
        ...
        for (let ref in process.monitors) {
            ...
            this.send(mons['monitor'], new erlang_types_1.default.Tuple('DOWN', ref, mons['monitee'], mons['monitee'], reason));
        }
    }
    error(reason) {
    ...
    make_ref() {
        return new erlang_types_1.default.Reference();
    }
}
